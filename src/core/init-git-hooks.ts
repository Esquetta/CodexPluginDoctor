...
async function resolveHooksDirectory(rootPath: string): Promise<string> {
  ...
  return path.join(path.resolve(worktreeGitDir, commonDirValue), "hooks");
}
...
export async function removeGitHooks(targetPath: string): Promise<RemoveGitHooksResult> {
  ...
  const hooksDir = await resolveHooksDirectory(rootPath);
