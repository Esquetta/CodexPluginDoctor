...
describe("public repository readiness", () => {
  ...
  it("documents offline public directory submission preflight without implying portal approval", async () => {
    ...
    expect(readme).toContain("codex-plugin-doctor doctor submission archive <zip>");
    expect(readme).toContain("does not extract archive entries to disk");
    expect(actionGuide).toContain('submission-archive: ./plugin.zip');
    expect(actionGuide).toContain('require-submission-ready: "true"');
    expect(actionGuide).toContain("submission-archive-json-path");
    expect(actionGuide).toContain("submission-archive-summary-path");
    expect(actionGuide).toContain("exactly one submission mode");
    expect(actionGuide).toContain("does not extract archive entries to disk");
    expect(actionGuide).toContain("coverage: unavailable");
    expect(catalog).toContain("## Public Directory Archive Preflight Rules");
    expect(catalog).toContain("plugin.submission.archive.invalid_zip");
    expect(catalog).toContain("plugin.submission.archive.mcp_excluded");
    expect(catalog).toContain("plugin.submission.archive.screenshot_excluded");
